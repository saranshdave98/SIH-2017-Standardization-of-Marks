...
<CENTER>
      <font face="Arial, Helvetica, sans-serif" color="#3333CC" size=6><b><u>Percentiles</u></b></font><br><br>
...
<font face="Arial, Helvetica, sans-serif" color="#3333CC" size=2>(All Regions)</font>
    ...
    <?php require ( 'Functions.php'); function Scorecard(&$conn) { $i=0; $result=mysqli_query($conn, "SELECT * FROM `stud_data`"); if ($result->num_rows>0) { ?>
   <table style="border:2px solid black;border-collapse:collapse;" id="t1"  align="center" cellpadding="8" >
        ...
        <thead style="background-color:rgb(55,58,190)">
            <td style="border:2px solid black;color:white">STUDENT ID</td>
           <td style="border:2px solid black;color:white">STUDENT NAME</td>
            <td style="border:2px solid black;color:white">PHYSICS PERCENTILE</td>
               <td style="border:2px solid black;color:white">MATHEMATICS PERCENTILE</td>
                <td style="border:2px solid black;color:white">CHEMISTRY PERCENTILE</td>
                <td style="border:2px solid black;color:white">ENGLISH PERCENTILE</td>
                <td style="border:2px solid black;color:white">COMPUTER PERCENTILE</td>
        </thead>


        <?php while($row=mysqli_fetch_array($result)) { $ID=$row[ "STUDENT_ID"]; $Name=$row[ "STUDENT_NAME"]; $PhyPerc=$row[ "PHYSICS_PERCENTILE"]; $ChemPerc=$row[ "CHEMISTRY_PERCENTILE"]; $MathPerc=$row[ "MATHEMATICS_PERCENTILE"]; $EngPerc=$row[ "ENGLISH_PERCENTILE"]; $CompPerc=$row[ "COMPUTER_PERCENTILE"]; ?>
        <?php if($i%2==0) { ?>
        <tr>
            ...
            <td style="border:2px solid black;">
                <?php echo round($PhyPerc,4);?>
            </td>
            <td style="border:2px solid black;">
                <?php echo round($MathPerc,4);?>
            </td>
            <td style="border:2px solid black;">
                <?php echo round($ChemPerc,4);?>
            </td>
            <td style="border:2px solid black;">
                <?php echo round($EngPerc,4);?>
            </td>
            <td style="border:2px solid black;">
                <?php echo round($CompPerc,4);?>
            </td>
        </tr>
    ...
    <?php } else { echo "SORRY NO DATABSE COMPUTED"; } } $conn=getConnection(); Scorecard($conn); ?>
 <br>
    <br>
 <br>
    <br>
